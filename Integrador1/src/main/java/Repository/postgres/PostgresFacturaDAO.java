...
public class PostgresFacturaDAO implements FacturaDAO {

    private static PostgresFacturaDAO instance;
    private Connection conn;

    private PostgresFacturaDAO(Connection conn) {
        this.conn = conn;
    }

    public static synchronized PostgresFacturaDAO getInstance(Connection conn) {
        if (instance == null) {
            instance = new PostgresFacturaDAO(conn);
        }
        return instance;
    }
    ...
    public void insertarDatos(ArrayList<Factura> fac) {
        ...
        try {
            ...
            ps.executeBatch();
            conn.commit();
            System.out.println("Datos de Factura cargados con exito en Postgres!");
        } catch (Exception e) {
