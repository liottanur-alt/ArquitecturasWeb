...
public class PostgresFacturaProductoDAO implements FacturaProductoDAO {

    private static PostgresFacturaProductoDAO instance;
    private Connection conn;

    private PostgresFacturaProductoDAO(Connection conn) {
        this.conn = conn;
    }

    public static synchronized PostgresFacturaProductoDAO getInstance(Connection conn) {
        if (instance == null) {
            instance = new PostgresFacturaProductoDAO(conn);
        }
        return instance;
    }
    ...
    public void insertarDatos(ArrayList<FacturaProducto> fps) {
        ...
        try {
            ...
            ps.executeBatch();
            conn.commit();
            System.out.println("Datos de FacturaProducto cargados con exito en Postgres!");
        } catch (Exception e) {
    ...
    public ArrayList<FacturaProducto> getFacturasProductos() {
        ...
        try (PreparedStatement sentencia = conn.prepareStatement(sql);
            ...
            throw new RuntimeException("Error al obtener facturas y productos", e);
        }
    ...
    private FacturaProducto mapear(ResultSet resultado) throws SQLException {
        ...
        facturaProducto.setIdFactura(resultado.getInt("idFactura"));
        facturaProducto.setIdProducto(resultado.getInt("idProducto"));
        facturaProducto.setCantidad(resultado.getInt("cantidad"));
